...
const CountUp = ({ value }: { value: number }) => {
  ...
  useEffect(() => {
    ...
    const controls = animate(0, value, {
      duration: Math.min(1.2, 0.4 + value / 2500),
      ease: [0.22, 1, 0.36, 1],
...
const PINNED_REPOS: GithubRepo[] = [
  ...
  },
  {
    ...
    description: 'Flicker-free streaming Markdown that renders partial LLM token streams progressively, handling unclosed syntax gracefully.',
    stars: 3,
    forks: 0,
    language: 'Dart',
    url: 'https://github.com/NarekManukyan/flutter_markdown_stream',
  },
  {
...
const SkeletonCard = () => (
  <div className="card p-6 animate-pulse">
    <div className="h-5 w-2/3 bg-surface-2 rounded mb-3" />
    <div className="h-3 w-full bg-surface-2 rounded mb-2" />
    <div className="h-3 w-4/5 bg-surface-2 rounded mb-4" />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="mb-20">
          <motion.h3
            ...
            transition={{ ...spring.entrance }}
            viewport={{ once: true }}
            className="text-xl font-semibold mb-8 text-ink flex items-center"
          >
            <svg className="w-5 h-5 mr-2.5 text-accent" fill="currentColor" viewBox="0 0 24 24" aria-label="GitHub icon">
            ...
            >
              {githubRepos.map((repo) => (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  whileTap={{ scale: 0.98 }}
                  className="card p-6 hover:border-accent-line transition-colors duration-300 flex flex-col"
                >
                  <h4 className="text-lg font-semibold text-ink mb-2">{repo.name}</h4>
                  <p className="text-ink-muted text-sm mb-4 flex-1">{repo.description}</p>
                  {repo.contribution != null && repo.contribution !== '' && (
                    <p className="text-sm text-ink-faint mb-4">{repo.contribution}</p>
                  )}
                  <div className="flex flex-wrap items-center gap-4 mb-4 tnum">
                    <span className="flex items-center text-sm text-ink-muted font-mono">
                      <svg className="w-4 h-4 mr-1.5 text-warm" fill="currentColor" viewBox="0 0 24 24" aria-label="Stars">
                        <title>Stars</title>
                    ...
                    <span className="flex items-center text-sm text-ink-muted font-mono">
                      <svg className="w-4 h-4 mr-1.5 text-ink-faint" fill="currentColor" viewBox="0 0 24 24" aria-label="Forks">
                        <title>Forks</title>
                    ...
                    <span className="text-xs font-mono text-ink-faint">{repo.language}</span>
                  </div>
                  <a
                    href={repo.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center text-sm font-medium text-accent hover:text-accent-strong transition-colors duration-200"
                  >
                    View on GitHub
        ...
        <div>
          <motion.h3
            ...
            className="text-xl font-semibold mb-8 text-ink flex items-center"
          >
            <svg className="w-5 h-5 mr-2.5" viewBox="0 0 502.87 502.87">
              <defs>
                <style>
          ...
          {loading && pubPackages.length === 0 && !pubError ? (
            ...
            <p className="text-ink-muted col-span-full">
            ...
            >
              {pubPackages.map((pkg) => (
                <motion.div
                  ...
                  className="card p-6 hover:border-accent-line transition-colors duration-300 flex flex-col"
                >
                  <h4 className="text-lg font-semibold text-ink mb-2">{pkg.name}</h4>
                  <p className="text-ink-muted text-sm mb-4 flex-1">{pkg.description}</p>
                  <div className="flex flex-wrap items-center gap-4 mb-4 tnum">
                    <span className="flex items-center text-sm text-ink-muted font-mono">
                      <svg className="w-4 h-4 mr-1.5 text-warm" fill="currentColor" viewBox="0 0 24 24" aria-label="Likes">
                        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                      </svg>
                      <CountUp value={pkg.likes} />
                    </span>
                    <span className="text-sm text-ink-muted font-mono">{pkg.points} pts</span>
                    <span className="text-sm text-ink-muted font-mono"><CountUp value={pkg.downloads} /> / 30d</span>
                  </div>
                  {pkg.features.length > 0 && (
                    <div className="mb-4">
                      <h5 className="text-sm font-semibold text-ink-muted mb-2">Key Features:</h5>
